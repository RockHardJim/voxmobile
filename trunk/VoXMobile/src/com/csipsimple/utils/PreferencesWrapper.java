...
public class PreferencesWrapper {
		...
		private static final long serialVersionUID = 1L;
	{
		...
		put(SipConfigManager.STUN_SERVER, ServiceHelper.getStunServer());
	...
	public String getUserAgent(Context ctx) {
		...
		//if(userAgent.equalsIgnoreCase(CustomDistribution.getUserAgent())) {
			...
			if(pinfo != null) {
				userAgent +=  " v" + pinfo.versionName + " r" + pinfo.versionCode+" / "+android.os.Build.DEVICE+"-"+Compatibility.getApiLevel();			
			}
		//}
		return userAgent;
	}
	...
	private static File getStorageFolder() {
	    ...
	    if (root.canWrite()){
			File dir = new File(root.getAbsolutePath() + File.separator + "VoXMobile");
			if(!dir.exists()) {
				dir.mkdirs();
