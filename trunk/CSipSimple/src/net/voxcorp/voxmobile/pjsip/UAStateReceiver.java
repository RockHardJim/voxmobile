...
package net.voxcorp.voxmobile.pjsip;
...
import net.voxcorp.voxmobile.api.SipCallSession;
import net.voxcorp.voxmobile.api.SipCallSession.StatusCode;
import net.voxcorp.voxmobile.api.SipConfigManager;
import net.voxcorp.voxmobile.api.SipManager;
import net.voxcorp.voxmobile.api.SipManager.PresenceStatus;
import net.voxcorp.voxmobile.api.SipMessage;
import net.voxcorp.voxmobile.api.SipProfile;
import net.voxcorp.voxmobile.api.SipUri;
import net.voxcorp.voxmobile.api.SipUri.ParsedSipContactInfos;
import net.voxcorp.voxmobile.service.MediaManager;
import net.voxcorp.voxmobile.service.SipNotifications;
import net.voxcorp.voxmobile.service.SipService;
import net.voxcorp.voxmobile.service.SipService.SameThreadException;
import net.voxcorp.voxmobile.service.SipService.SipRunnable;
import net.voxcorp.voxmobile.service.impl.SipCallSessionImpl;
import net.voxcorp.voxmobile.utils.CallLogHelper;
import net.voxcorp.voxmobile.utils.Compatibility;
import net.voxcorp.voxmobile.utils.Log;
import net.voxcorp.voxmobile.utils.Threading;
import net.voxcorp.voxmobile.utils.TimerWrapper;
import net.voxcorp.voxmobile.vox.utils.VoXMobileSipApi;
    ...
    public void on_call_state(final int callId, pjsip_event e) {
        ...
        try {
            ...
            if (callState == SipCallSession.InvState.DISCONNECTED) {
                ...
                // Handle VoX Mobile specific responses
                VoXMobileSipApi.handleNegativeSipStatus(pjService, callInfo);
            } else {
                if (ongoingCallLock != null && !ongoingCallLock.isHeld()) {
                    ongoingCallLock.acquire();
                }
                ...
                case ON_CALL_STATE: {
                    ...
                    switch (callState) {
                        ...
                        case SipCallSession.InvState.DISCONNECTED:
                            ...
                            /*
                             * VoX Mobile :: Set call log text for negative status codes
                             */
                            VoXMobileSipApi.setCallLogStatusCodeText(stateReceiver.pjService.service, callInfo, cv);
                            ...
                            if (callInfo.getLastStatusCode() != 200 && callInfo.getLastReasonCode() != 200) {
                            	/*
                            	 * VoX Mobile :: Don't show Toast messages for FreeSWITCH;Q.850 responses
                            	 */
                            	if (callInfo.getLastStatusCode() < 900)
                            	{
                            ...
                            /* VoX Mobile :: No saving when remote contact is missing
                             * Don't write call log if remoteContact is null because this probably
                             * means a VoXLand user tried to call the PSTN and got a SIP 503 response
                             */
                            boolean voxMobileSaveFlag = callInfo.getRemoteContact() != null;

                            // If needed fill native database
                            if (voxMobileSaveFlag && stateReceiver.mIntegrateWithCallLogs) {
                            ...
                            /*
                             * VoX Mobile :: Extra disconnect stuff
                             */
                            if(callInfo.getAccId() != SipProfile.INVALID_ID) {
                            	Log.d(THIS_FILE, "VoX Mobile disconnect handler");
                            	VoXMobileSipApi.handleSipDisconnect(stateReceiver.pjService.service, callInfo);
                            }
    ...
    public void initService(PjSipService srv) {
        ...
        if (eventLock == null) {
            ...
            eventLock = pman.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,
                    "net.voxcorp.voxmobile.inEventLock");
        ...
        if (ongoingCallLock == null) {
            ...
            ongoingCallLock = pman.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,
                    "net.voxcorp.voxmobile.ongoingCallLock");
