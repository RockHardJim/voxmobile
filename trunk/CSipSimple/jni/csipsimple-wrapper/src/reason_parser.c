...
/**
 * VoX Mobile :: Get event status code of an event. Including Q.850 processing
 */
PJ_DECL(int) get_event_status_code(pjsip_event *e) {
  ...
  int retval = get_vox_error_info(e);
  if (retval > 0) {
    return retval;
  }

	retval = get_reason_code(e, "FreeSWITCH", &lookup_q850_cause);
	if (retval > 0) {
		return retval;
	} else {
