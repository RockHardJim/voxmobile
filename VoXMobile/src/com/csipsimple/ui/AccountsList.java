...
import com.csipsimple.voxmobile.ui.MainMenuActivity;
import com.csipsimple.voxmobile.ui.ProvisionWaitActivity;
import com.csipsimple.voxmobile.ui.StartActivity;
...
public class AccountsList extends Activity implements OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//Add add row
		LinearLayout add_row = (LinearLayout) findViewById(R.id.add_voxmobile_account);
		add_row.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				if (!isOnline()) {
					AlertDialog alertDialog = new AlertDialog.Builder(AccountsList.this).create();
					alertDialog.setTitle(R.string.voxmobile_attention);
					alertDialog.setMessage(getString(R.string.connection_not_valid));
		...
		//Add add row
		add_row = (LinearLayout) findViewById(R.id.add_account);
		add_row.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				clickAddAccount();
	...
	private int getVoXMenuText() {
		String status = OrderHelper.getProvisionStatus(this);
		
		if (status.equals(OrderHelper.PROVISION_STATUS_WAITING)) {
			return R.string.voxmobile_check_status;
		} else if ("".equals(VoXMobile.getUuid(this))) {
			return R.string.add_voxmobile_account;			
		} else {
			return R.string.voxmobile_manage;
		}
	}
	
	private int getVoXMenuTextColor() {
		String status = OrderHelper.getProvisionStatus(this);
	...
	protected void onResume() {
		TextView voxItem = (TextView) findViewById(R.id.add_voxmobile_item);
		voxItem.setText(getVoXMenuText());
		voxItem.setTextColor(getVoXMenuTextColor());
		super.onResume();
	}
    ...
    private void clickAddVoXMobileAccount() {	
		String status = OrderHelper.getProvisionStatus(this);
		
		if (status.equals(OrderHelper.PROVISION_STATUS_WAITING)) {
			startActivity(new Intent(AccountsList.this, ProvisionWaitActivity.class));
		} else if ("".equals(VoXMobile.getUuid(this))) {
			startActivity(new Intent(AccountsList.this, StartActivity.class));
		} else {
			startActivity(new Intent(AccountsList.this, MainMenuActivity.class));
		}
    }
	...
	public static final int ADD_VOX = Menu.FIRST + 4;

	@Override
	public boolean onPrepareOptionsMenu(Menu menu) {
		for (int i = 0; i < menu.size(); i++) {
			MenuItem m = menu.getItem(i);
			if (m.getItemId() == ADD_VOX) {
				menu.getItem(0).setTitle(getVoXMenuText());
				break;
			}
		}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		menu.add(Menu.NONE, ADD_VOX, Menu.NONE, getVoXMenuText()).setIcon(android.R.drawable.ic_menu_add);
